...
func (w *WebSocketReporter) routeCommand(cmd CommandMessage) {
	...
	case "QualityProbe":
		...
		response.Type = "QualityProbeResponse"
		response.Data = probeResult

	case "PublicIpQuery":
		var publicIPResult publicIPResponse
		publicIPResult, err = w.handlePublicIPQuery(cmd.Data)
	...
	case "LanDiscovery":
		...
		response.Type = "LanDiscoveryResponse"
		response.Data = discoveryResult

	case "NodeServiceDiscovery":
		var discoveryResult nodeServiceDiscoveryResponse
		discoveryResult, err = w.handleNodeServiceDiscovery(cmd.Data)
...
func StartWebSocketReporterWithConfig(addr string, secret string, http int, tls int, socks int, version string, role string) *WebSocketReporter {
	connectionType := "1"
	if role == "connector" {
		connectionType = "2"
	}
