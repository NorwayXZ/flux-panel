CREATE TABLE IF NOT EXISTS authorized_entry_template (
  ...
  KEY idx_authorized_entry_template_group (source_group_id),KEY idx_authorized_entry_template_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS authorized_entry_grant (
  ...
  KEY idx_authorized_entry_grant_user (user_id,state),KEY idx_authorized_entry_grant_cycle (state,expires_at,flow_reset_day)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS authorized_entry_port (
  ...
  last_error varchar(500) DEFAULT NULL,created_time bigint NOT NULL,updated_time bigint NOT NULL,PRIMARY KEY (id),
  UNIQUE KEY uk_authorized_entry_grant_port (grant_id,port),KEY idx_authorized_entry_port_user (user_id,state)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS authorized_entry_forward (
  id bigint unsigned NOT NULL AUTO_INCREMENT,port_id bigint NOT NULL,forward_id bigint NOT NULL,node_id bigint NOT NULL,
  created_time bigint NOT NULL,updated_time bigint NOT NULL,PRIMARY KEY (id),
  UNIQUE KEY uk_authorized_entry_forward (forward_id),KEY idx_authorized_entry_port_forward (port_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
