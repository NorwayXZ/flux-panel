...
const resourceIcon = (type: MonitoringResourceType, className = "h-4 w-4") => {
  ...
  if (type === "certificate" || type === "dynamic_dns")
...
export default function MonitoringPage() {
  ...
  const [view, setView] = useState<
    "alerts" | "resources" | "notifications" | "timing"
  >("alerts");
  ...
  const [typeFilter, setTypeFilter] = useState<"all" | MonitoringResourceType>(
    "all",
  );
  ...
  const [apiTiming, setApiTiming] =
    useState<ApiTimingOverview>(EMPTY_API_TIMING);
  const [apiTimingLoading, setApiTimingLoading] = useState(false);
  const isAdmin = isAdminUser();

  const loadOverview = useCallback(
    async (selectedRange: MonitoringRange, silent = false) => {
      ...
      try {
        ...
        if (!silent)
          toast.error(
            error instanceof Error ? error.message : "获取监控概览失败",
          );
      } finally {
  ...
  const loadAlerts = useCallback(
    async (silent = false) => {
      try {
        ...
        if (response.code !== 0)
          throw new Error(response.msg || "获取告警列表失败");
        setAlerts(response.data || EMPTY_ALERTS);
        notifyAlertCountChanged();
      } catch (error) {
        if (!silent)
          toast.error(
            error instanceof Error ? error.message : "获取告警列表失败",
          );
      }
    },
  ...
  const loadApiTiming = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (response.code !== 0)
        throw new Error(response.msg || "获取接口性能数据失败");
      setApiTiming(response.data || EMPTY_API_TIMING);
    } catch (error) {
      if (!silent)
        toast.error(
          error instanceof Error ? error.message : "获取接口性能数据失败",
        );
    } finally {
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      meta:
        overview.summary.criticalAlerts > 0 ? "需要立即处理" : "当前无严重故障",
      ...
      <div className="mx-auto max-w-[1500px] space-y-5">
        ...
        >
          {summaryCards.map((card) => (
            <Card
              key={card.label}
              className="border border-gray-200 dark:border-gray-800 dark:bg-gray-950"
              radius="sm"
              shadow="sm"
            >
              <CardBody className="gap-3 p-4 sm:p-5">
                ...
                <div>
                  ...
                  <p className="mt-1 truncate text-xs text-default-500 sm:text-sm">
                    {card.meta}
                  </p>
                </div>
              </CardBody>
        ...
        <section
          aria-label="在线率趋势"
          className="border-y border-gray-200 bg-white px-3 py-5 dark:border-gray-800 dark:bg-gray-950 sm:px-5"
        >
          <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-default-500">
                {overview.summary.trackedFrom
                  ? `数据起始 ${formatDateTime(overview.summary.trackedFrom)}`
                  : "正在建立历史基线"}
              </p>
            </div>
        ...
        <section className="space-y-4">
          ...
          {view === "timing" && isAdmin ? (
            <ApiTimingPanel
              data={apiTiming}
              loading={apiTimingLoading}
              onRefresh={() => loadApiTiming()}
            />
...
function ApiTimingPanel({
  ...
  const summaryCards = [
    ...
    [
      "慢请求",
      data.summary.slowCount,
      `阈值 ${formatTimingMs(data.thresholdMs)}`,
    ],
  ...
  return (
    <div className="space-y-4">
      ...
      {loading && data.routes.length === 0 ? (
        <div className="flex min-h-48 items-center justify-center border-y border-divider bg-content1">
          <Spinner />
        </div>
      ...
      <p className="text-xs text-default-500">
        最后采样时间：
        {data.capturedAt ? formatDateTime(data.capturedAt) : "暂无"}
        。重启后端后统计窗口会重新开始。
      </p>
    </div>
  );
...
function ApiTimingRow({
  route,
  rank,
}: {
  route: ApiTimingRoute;
  rank: number;
}) {
    ...
    <tr className={severe ? "bg-danger-50/40 dark:bg-danger-500/5" : undefined}>
      <td className="max-w-[360px] px-4 py-3 align-top">
        <div className="flex items-start gap-3">
          <span className="flex h-6 w-6 flex-none items-center justify-center rounded-full bg-default-100 text-xs font-semibold">
            {rank}
          </span>
      ...
      <td
        className={`px-4 py-3 align-top font-semibold ${severe ? "text-danger" : "text-foreground"}`}
      >
      ...
      <td className="px-4 py-3 align-top text-xs text-default-500">
        {formatTimingMs(route.lastMs)} · {formatDateTime(route.lastAt)}
        <br />
        HTTP {route.lastStatus}
      </td>
    </tr>
  );
}
...
function NotificationSettings() {
  ...
  const test = async () => {
    ...
    response.code === 0
      ...
      : toast.error(response.msg || "测试通知发送失败");
  };

  const update = <K extends keyof TelegramNotificationSettings>(
    key: K,
    value: TelegramNotificationSettings[K],
  ) => {
  ...
  const eventRows: Array<{
  ...
  }> = [
    {
      label: "节点异常",
      detail: "Agent 离线或恢复",
      enabledKey: "nodeEnabled",
      repeatKey: "nodeRepeatLimit",
    },
    {
      label: "隧道异常",
      detail: "链路离线、降级或恢复",
      enabledKey: "tunnelEnabled",
      repeatKey: "tunnelRepeatLimit",
    },
    {
      label: "转发异常",
      detail: "线路或目标不可用、恢复",
      enabledKey: "forwardEnabled",
      repeatKey: "forwardRepeatLimit",
    },
  ];
    ...
    <div className="overflow-hidden border-y border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950">
      ...
      <div className="grid gap-4 border-b border-divider px-4 py-5 sm:px-5 lg:grid-cols-2">
        ...
        <Input
          ...
          onValueChange={(value) => update("chatId", value)}
        />
      </div>
      ...
      <div className="border-b border-divider px-4 py-5 sm:px-5">
        <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-default-500">
              同一故障按设定次数发送，具体对象单独计数。
            </p>
          </div>
          <Switch
            isSelected={settings.recoveryEnabled}
            size="sm"
            onValueChange={(value) => update("recoveryEnabled", value)}
          >
            恢复通知
          </Switch>
        </div>
        <div className="divide-y divide-divider border-y border-divider">
          {eventRows.map((row) => (
            ...
            >
              ...
              <Switch
                isSelected={settings[row.enabledKey] as boolean}
                size="sm"
                onValueChange={(value) => update(row.enabledKey, value)}
              >
              ...
              <label className="flex items-center gap-2 text-sm text-default-500">
                ...
                <select
                  ...
                  onChange={(event) =>
                    update(row.repeatKey, Number(event.target.value))
                  }
                >
                  {[1, 2, 3, 4, 5].map((value) => (
          ...
          <div className="grid gap-3 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
            <div>
              <p className="font-medium text-foreground">动态 DNS</p>
              <p className="mt-1 text-sm text-default-500">
                更新失败和恢复，每次故障只通知一次
              </p>
            </div>
            <Switch
              isSelected={settings.dynamicDnsEnabled}
              size="sm"
              onValueChange={(value) => update("dynamicDnsEnabled", value)}
            >
              {settings.dynamicDnsEnabled ? "通知" : "关闭"}
            </Switch>
          </div>
          <div className="grid gap-3 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
            <div>
              <p className="font-medium text-foreground">服务器到期</p>
              <p className="mt-1 text-sm text-default-500">
                按资产中心设置的提前天数提醒
              </p>
            </div>
            <Switch
              isSelected={settings.assetExpiryEnabled}
              size="sm"
              onValueChange={(value) => update("assetExpiryEnabled", value)}
            >
              {settings.assetExpiryEnabled ? "通知" : "关闭"}
            </Switch>
          </div>
        </div>
          ...
          <Input
            ...
            onValueChange={(value) =>
              update(
                ...
                Math.max(5, Math.min(1440, Number(value) || 5)),
              )
            }
          />
        </div>
        ...
        <div>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-default-500">
                登录成功后检查来源地址，同账号同地址在冷却期内只发一次。
              </p>
            </div>
            <Switch
              isSelected={settings.loginOutsideWhitelistEnabled}
              size="sm"
              onValueChange={(value) =>
                update("loginOutsideWhitelistEnabled", value)
              }
            >
...
function AlertList(props: AlertListProps) {
  const totalPages = Math.max(
    1,
    Math.ceil(props.alerts.total / props.alerts.size),
  );
  const controlClass =
    "h-10 rounded-md border border-gray-200 bg-white px-3 text-sm text-foreground outline-none focus:border-blue-500 dark:border-gray-700 dark:bg-gray-950";
      ...
      <div className="flex flex-col gap-2 lg:flex-row">
        <div className="grid grid-cols-3 gap-2 lg:flex">
          <select
            ...
            onChange={(event) =>
              props.onStatusChange(
                event.target.value as AlertListProps["statusFilter"],
              )
            }
          >
          ...
          <select
            ...
            onChange={(event) =>
              props.onTypeChange(
                event.target.value as AlertListProps["typeFilter"],
              )
            }
          >
          ...
          <select
            ...
            onChange={(event) =>
              props.onSeverityChange(
                event.target.value as AlertListProps["severityFilter"],
              )
            }
          >
        ...
        >
          ...
          />
          <button
            className="absolute right-1 top-1 h-8 rounded px-3 text-sm font-medium text-blue-600 hover:bg-blue-50 dark:text-blue-300 dark:hover:bg-blue-500/10"
            type="submit"
          >
            搜索
          </button>
      ...
      <div className="overflow-hidden border-y border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950 sm:rounded-md sm:border">
        {props.alerts.items.length === 0 ? (
          ...
          props.alerts.items.map((alert, index) => (
            ...
            >
              <span
                className={`mt-0.5 flex h-9 w-9 items-center justify-center rounded-md sm:mt-0 ${
              ...
              <span className="min-w-0">
                ...
                <span className="mt-1 block text-sm text-default-500">
                  {alert.detail || "未提供故障详情"}
                </span>
                <span className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-xs text-default-400">
                  <span>
                    {RESOURCE_LABELS[alert.resourceType]} ID {alert.resourceId}
                  </span>
                  {props.isAdmin && <span>所属：{alert.ownerUserName}</span>}
                  <span>
                    持续：{formatDuration(alert.startedAt, alert.resolvedAt)}
                  </span>
                </span>
              </span>
              <span className="text-xs text-default-400 sm:text-right">
                ...
                {alert.resolvedAt && (
                  <span className="mt-1 block text-emerald-600 dark:text-emerald-400">
                    恢复 {formatDateTime(alert.resolvedAt)}
                  </span>
                )}
              </span>
            </button>
      ...
      {props.alerts.total > props.alerts.size && (
        <div className="flex items-center justify-between text-sm text-default-500">
          ...
          <div className="flex items-center gap-2">
            <Button
              isDisabled={props.alerts.page <= 1}
              size="sm"
              variant="flat"
              onPress={() => props.onPageChange(props.alerts.page - 1)}
            >
              上一页
            </Button>
            <span>
              {props.alerts.page}/{totalPages}
            </span>
            <Button
              isDisabled={props.alerts.page >= totalPages}
              size="sm"
              variant="flat"
              onPress={() => props.onPageChange(props.alerts.page + 1)}
            >
          ...
          >
            <div className="flex min-w-0 items-center gap-3">
              ...
              <span className="min-w-0">
                ...
                <span className="mt-0.5 block truncate text-xs text-default-400">
                  {RESOURCE_LABELS[resource.type]} ID {resource.id}
                  {isAdmin ? ` · ${resource.ownerUserName}` : ""}
                </span>
              </span>
            </div>
            <div>
              <span
                className={`inline-flex rounded border px-2 py-1 text-xs font-medium ${statusStyles[resource.status]}`}
              >
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-default-400">
                状态变化 {formatDateTime(resource.changedAt)}
              </p>
            </div>
            <div>
              <p className="text-sm font-semibold text-foreground">
                {resource.availability.toFixed(2)}%
              </p>
              <div className="mt-1.5 h-1.5 overflow-hidden rounded bg-gray-100 dark:bg-gray-800">
                <div
                  ...
                  style={{
                    width: `${Math.max(0, Math.min(resource.availability, 100))}%`,
                  }}
                />
              </div>
            </div>
            <div className="flex items-center justify-between lg:block">
              <span className="text-xs text-default-400 lg:hidden">
                故障次数
              </span>
              <span className="text-sm font-medium text-foreground">
                {resource.incidentCount}
              </span>
            </div>
          </div>
        ))
