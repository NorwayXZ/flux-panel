...
export default function TunnelPage() {
  ...
  const handleEdit = (tunnel: Tunnel) => {
    ...
    setForm({
      ...
      nodePath:
        getTunnelNodePath(tunnel).length > 0
          ? getTunnelNodePath(tunnel)
          : [tunnel.inNodeId, tunnel.outNodeId || null],
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (response.code === 0) {
        ...
        const forwardGostCleanupFailCount = Number(
          summary.forwardGostCleanupFailCount || 0,
        );
        const userTunnelCount = Number(summary.userTunnelCount || 0);
        const speedLimitCount = Number(summary.speedLimitCount || 0);
        const cleanupText =
          forwardCount + userTunnelCount + speedLimitCount > 0
            ? `，已清理 ${forwardCount} 个转发、${userTunnelCount} 个授权、${speedLimitCount} 个限速规则`
            : "";
        const remoteCleanupText =
          forwardGostCleanupFailCount > 0
  ...
  const normalizeFormPath = (
    ...
    const path =
      nodePath.length > 0 ? [...nodePath] : [inNodeId, outNodeId ?? null];
  ...
  const getTunnelNodePath = (tunnel: Tunnel): number[] => {
    ...
    return [tunnel.inNodeId, tunnel.outNodeId].filter(
      (nodeId): nodeId is number => !!nodeId,
    );
  };
  ...
  const getLinkStatusText = (tunnel: Tunnel): string => {
    ...
    return tunnel.type === 1
      ? "入口节点正常"
      : `${pathNodes.length} 跳链路正常`;
  };
  ...
  const updateFormPathNode = (index: number, nodeId: number | null) => {
    setForm((prev) => {
      const nextPath = normalizeFormPath(
        prev.inNodeId,
        prev.nodePath,
        prev.outNodeId,
      );
      ...
      const nextOutNodeId =
        nextPath.length > 1 ? nextPath[nextPath.length - 1] : null;
  ...
  const tunnelCardOrder = useCardOrder(
    "tunnel-cards",
  ...
  const onlineTunnels = tunnelCardOrder.sortItems(
    tunnels
      .filter(
        (tunnel) =>
          !isTunnelNodeOffline(tunnel) &&
          !(tunnel.accessType === "shared" && tunnel.quotaAvailable === false),
      )
      .sort(compareCreatedTimeDesc),
    (tunnel) => tunnel.id,
  );
  ...
  const groupTunnelsByLevel = (tunnelList: Tunnel[]): TunnelLevelGroup[] => {
    ...
    tunnelList.forEach((tunnel) => {
      ...
      const level = isPortForward
        ? 1
        : Math.max(getTunnelNodePath(tunnel).length, 2);
    ...
    <SortableCardGrid
      ...
      renderItem={(tunnel, dragHandle) => {
        ...
        return (
          <Card
            className={
              tunnelOffline || permissionUnavailable
          ...
          >
            {(tunnelOffline || permissionUnavailable) && (
              <div className="offline-accent h-0.5 bg-danger" />
            )}
            <CardHeader className="pb-2">
              <div className="flex justify-between items-start w-full">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 min-w-0">
                    <h3 className="font-semibold text-foreground truncate text-sm flex-1 min-w-0">
                      {tunnel.name}
                    </h3>
                    <Chip
              ...
              <div className="space-y-2">
                {tunnel.accessType === "shared" && (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2 text-xs">
                      ...
                      >
                        {permissionUnavailable
                          ? tunnel.unavailableReason
                          : "资源额度正常"}
                      </span>
                      <span className="text-default-500 whitespace-nowrap">
                        {tunnel.quotaFlowUnlimited
                          ? "流量不限"
                          : `${((tunnel.quotaUsedFlow || 0) / 1073741824).toFixed(1)} / ${tunnel.quotaFlow || 0} GB`}
                      </span>
                    </div>
                    <div className="mt-1 text-[11px] text-default-500">
                      转发名额：
                      {tunnel.quotaForwardUnlimited
                        ? "不限"
                        : `${tunnel.quotaForwardUsed || 0} / ${tunnel.quotaForwardLimit || 0}`}
                    </div>
                  </div>
                )}
                {/* 流程展示 */}
                <div className="space-y-1.5">
                  {pathNodes.map((pathNode, index) => {
                    ...
                    return (
                      <div
                        key={`${tunnel.id}-${pathNode.nodeId}-${index}`}
                        ...
                        <div className={getNodeBlockClassName(nodeOffline)}>
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <Chip
                              className={
                                nodeOffline
                                  ? "text-xs offline-status-chip"
                                  : "text-xs"
                              }
                              color={nodeChip.color as any}
                              size="sm"
                              variant="flat"
                            >
                              {nodeChip.text}
                          ...
                          <code
                            className={
                              nodeOffline
                                ? "text-xs font-mono text-danger-800 dark:text-danger-200 block truncate"
                                : "text-xs font-mono text-foreground block truncate"
                            }
                          >
                            {pathNode.name}
                          </code>
                          <code
                            className={
                              nodeOffline
                                ? "text-xs font-mono text-danger-600 dark:text-danger-300 block truncate"
                                : "text-xs font-mono text-default-500 block truncate"
                            }
                          >
                        ...
                        {index < pathNodes.length - 1 && (
                          <div className="py-0.5 text-center">
                            ...
                            {tunnel.hopDetails?.[index] && (
                              <div
                                className="mt-1 truncate text-[10px] text-default-500"
                                title={`${tunnel.hopDetails[index].addressModeName} ${tunnel.hopDetails[index].targetAddress}`}
                              >
                                {tunnel.hopDetails[index].addressModeName} ·{" "}
                                {tunnel.hopDetails[index].targetAddress}
                                {tunnel.hopDetails[index].fallbackAddress
                                  ? " · 公网备用"
                                  : ""}
                              </div>
                            )}
                ...
                <div
                  className={
                    tunnelOffline
                      ? "flex justify-between items-center pt-2 border-t border-danger-200 dark:border-danger-800"
                      : "flex justify-between items-center pt-2 border-t border-divider"
                  }
                >
            ...
            <section className="space-y-3">
              <div className="offline-section-divider flex items-center justify-between border-b border-danger-200 pb-2">
                ...
                <Chip
                  ...
                  variant="flat"
                >
                  {quotaUnavailableTunnels.length} 条
                </Chip>
              </div>
                          ...
                          .map((nodeId, offset, tail) => {
                            ...
                            return (
                              ...
                              >
                                <Select
                                  errorMessage={
                                    isLast ? errors.nodePath : undefined
                                  }
                                  isDisabled={isEdit}
