...
const ruleInitial = { id: undefined as number | undefined, name: '', sourceType: 'node' as 'node' | 'connector', nodeId: '', connectorId: '', providerKey: '', zoneName: '', recordName: '', recordType: 'A' as 'A' | 'AAAA', ttl: '600', checkIntervalSeconds: '60', enabled: true };
...
export default function DynamicDnsPage() {
  ...
  const load = async () => {
    setLoading(true);
    const [overview, nodeResponse, connectorResponse] = await Promise.all([getDynamicDnsOverview(), getNodeList(), getInternalConnectors()]);
    ...
    if (connectorResponse.code === 0) setConnectors(connectorResponse.data || []);
    setLoading(false);
  };
  useEffect(() => { void load(); }, []);
  ...
  const openRule = (rule?: DynamicDnsRule) => {
    ...
    setRuleForm(rule ? { id: rule.id, name: rule.name, sourceType: rule.sourceType || 'node', nodeId: rule.nodeId ? String(rule.nodeId) : '', connectorId: rule.connectorId ? String(rule.connectorId) : '', providerKey: provider?.optionKey || `${rule.providerSource}:${rule.providerRefId}`, zoneName: rule.zoneName, recordName: rule.recordName, recordType: rule.recordType, ttl: String(rule.ttl), checkIntervalSeconds: String(rule.checkIntervalSeconds), enabled: Boolean(rule.enabled) } : ruleInitial);
    setRuleOpen(true);
  };
  const saveRule = async () => {
    const option = data.providers.find(item => item.optionKey === ruleForm.providerKey);
    if (!option || !ruleForm.recordName.trim()) return toast.error('请选择 DNS 配置并填写记录');
    if (ruleForm.sourceType === 'node' && !ruleForm.nodeId) return toast.error('请选择检测节点');
    if (ruleForm.sourceType === 'connector' && !ruleForm.connectorId) return toast.error('请选择家庭设备');
    setBusy('rule');
    const response = await saveDynamicDnsRule({ id: ruleForm.id, name: ruleForm.name, sourceType: ruleForm.sourceType, nodeId: ruleForm.sourceType === 'node' ? Number(ruleForm.nodeId) : undefined, connectorId: ruleForm.sourceType === 'connector' ? Number(ruleForm.connectorId) : undefined, providerSource: option.source, providerRefId: option.id, provider: option.provider, zoneRefId: option.zoneRefId, zoneName: option.zoneName || ruleForm.zoneName, recordName: ruleForm.recordName, recordType: ruleForm.recordType, ttl: Number(ruleForm.ttl), checkIntervalSeconds: Number(ruleForm.checkIntervalSeconds), enabled: ruleForm.enabled });
    setBusy(null);
  ...
  return <div className="mx-auto w-full max-w-[1500px] space-y-5 p-4 md:p-6">
    <AccessResourceTabs />
    <header className="flex flex-col gap-4 border-b border-divider pb-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm text-default-500">资源中心</p><h1 className="mt-1 text-2xl font-semibold">动态解析</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-default-500">本页只管理公网地址变化后的更新规则和执行历史；DNS 凭据统一优先在域名管理维护，旧的独立凭据仍可兼容使用。</p></div><div className="flex gap-2"><Button isIconOnly variant="flat" aria-label="刷新" onPress={load}><RefreshCw size={17} /></Button>{tab === 'rules' ? <Button color="primary" startContent={<Plus size={17} />} onPress={() => openRule()}>新建规则</Button> : <Button color="primary" startContent={<Plus size={17} />} onPress={() => navigate('/dns-settings')}>前往域名管理</Button>}</div></header>
    ...
    {tab === 'rules' ? <section className="overflow-hidden rounded-md border border-divider bg-content1">
      <div className="hidden grid-cols-[minmax(190px,1.3fr)_minmax(180px,1fr)_160px_150px_140px_136px] gap-4 border-b border-divider px-4 py-3 text-xs font-semibold text-default-500 lg:grid"><span>域名记录</span><span>检测来源</span><span>DNS 提供商</span><span>当前地址</span><span>状态</span><span className="text-right">操作</span></div>
      {loading ? <div className="flex min-h-48 items-center justify-center text-default-500">正在读取规则</div> : data.rules.length === 0 ? <div className="flex min-h-48 flex-col items-center justify-center gap-2 text-default-500"><CloudCog size={30} /><span>尚未创建动态 DNS 规则</span></div> : data.rules.map((rule, index) => <div key={rule.id} className={`grid gap-3 px-4 py-4 lg:grid-cols-[minmax(190px,1.3fr)_minmax(180px,1fr)_160px_150px_140px_136px] lg:items-center ${index ? 'border-t border-divider' : ''}`}>
        ...
        <div><p className="truncate font-medium">{rule.sourceType === 'connector' ? (rule.connectorName || `接入端 ${rule.connectorId}`) : (rule.nodeName || `节点 ${rule.nodeId}`)}</p><p className={`mt-1 text-xs ${rule.sourceType === 'connector' ? (rule.connectorOnline ? 'text-success' : 'text-danger') : (rule.nodeOnline ? 'text-success' : 'text-danger')}`}>{rule.sourceType === 'connector' ? (rule.connectorOnline ? '接入端在线' : '接入端离线') : (rule.nodeOnline ? '节点在线' : '节点离线')} · Agent {rule.sourceType === 'connector' ? (rule.connectorVersion || '未知') : (rule.nodeVersion || '未知')}</p></div>
