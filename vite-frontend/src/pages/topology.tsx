...
function ResourceNode({ data }: NodeProps<Node<TopologyResourceNode>>) {
  ...
  return (
    <div
      className={`h-[82px] w-[220px] rounded-md border bg-content1 px-3 py-2 shadow-sm ${meta.border}`}
    >
      <Handle
        className="!h-2 !w-2 !border-content1 !bg-default-400"
...
function TopologyCanvas() {
  ...
  const [resourceEdges, setResourceEdges] = useState<
    Array<{
      id: string;
      source: string;
      target: string;
  ...
  const load = useCallback(async () => {
    ...
    setSummary(
      response.data.summary || { nodes: 0, links: 0, healthy: 0, abnormal: 0 },
    );
  }, []);
    ...
    () =>
      resourceEdges
        ...
        .map((edge) => ({
          ...
          style: {
            stroke: ["offline", "failed"].includes(edge.status)
              ? "#f43f5e"
  ...
  const nodes = useMemo(
    ...
    [edges, resourceNodes, visible],
  );

  useEffect(() => {
    if (nodes.length === 0) return;
    const timer = window.setTimeout(
      () => flow.fitView({ padding: 0.16, duration: 300 }),
      80,
    );
  ...
  if (loading && resourceNodes.length === 0)
    return (
      <div className="flex h-[70vh] items-center justify-center">
        <Spinner label="加载全链路拓扑" />
      </div>
    );
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-divider pb-4">
        <div>
          <p className="text-sm text-default-500">资源关系与故障定位</p>
          <h1 className="mt-1 text-2xl font-semibold">全链路拓扑</h1>
        </div>
        <div className="flex items-center gap-2">
          <Tabs
            ...
            onSelectionChange={(key) =>
              setMode(String(key) as "all" | "abnormal")
            }
          >
      ...
      <div className="grid grid-cols-2 gap-px border-b border-divider bg-divider sm:grid-cols-4">
        {[
          ["资源组件", summary.nodes],
          ["连接关系", summary.links],
          ["运行正常", summary.healthy],
          ["异常组件", summary.abnormal],
        ].map(([label, value], index) => (
          <div key={String(label)} className="bg-background px-4 py-3">
            <div className="text-xs text-default-500">{label}</div>
            <div
              className={`mt-1 text-xl font-semibold ${index === 3 && Number(value) > 0 ? "text-danger" : ""}`}
            >
              {value}
            </div>
          </div>
      ...
      <div
        className="relative shrink-0 bg-default-50"
        style={{ height: "clamp(520px, calc(100dvh - 250px), 900px)" }}
      >
        {nodes.length === 0 ? (
          <div className="flex h-full min-h-[620px] flex-col items-center justify-center gap-3 text-default-500">
            <Network size={34} />
            <span>
              {mode === "abnormal"
                ? "当前没有异常链路"
          ...
          <ReactFlow<Node<TopologyResourceNode>, Edge>
            fitView
            ...
            onNodeClick={(_, node) => navigate(String(node.data.path))}
          >
            <Background color="rgba(113,113,122,.22)" gap={22} size={1} />
            <MiniMap
              pannable
              zoomable
              nodeColor={(node) => {
                const status = (node.data as unknown as TopologyResourceNode)
                  .status;

                return ["offline", "failed"].includes(status)
                  ? "#f43f5e"
                  : status === "degraded"
                    ? "#f59e0b"
                    : "#22c55e";
