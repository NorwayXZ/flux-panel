...
const getLatestVersion = async (): Promise<RemoteVersion | null> => {
  ...
  const versionTag = tags
    ...
    .sort((left, right) =>
      compareVersions(right.name || "", left.name || ""),
    )[0];
...
export default function UpdatePage() {
  ...
  const [remoteVersion, setRemoteVersion] = useState<RemoteVersion | null>(
    null,
  );
  const [errorMessage, setErrorMessage] = useState("");
  const [lastChecked, setLastChecked] = useState<Date | null>(null);
  const [systemStatus, setSystemStatus] = useState<SystemUpdateStatus | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (!stopped) {
        const active =
          systemStatus?.state === "queued" || systemStatus?.state === "running";
  ...
  const hasUpdate = useMemo(
    () =>
      remoteVersion
        ? compareVersions(remoteVersion.version, siteConfig.version) > 0
        : false,
    [remoteVersion],
  );
      ...
      <div className="mx-auto max-w-4xl space-y-5">
        ...
        <Card className="border border-default-200 shadow-sm">
          <CardBody className="space-y-4 p-5">
            ...
            {state === "success" && remoteVersion && (
              <div
                className={`flex flex-wrap items-center justify-between gap-4 rounded-lg border px-4 py-3 ${hasUpdate ? "border-warning-200 bg-warning-50 dark:border-warning-500/30 dark:bg-warning-500/10" : "border-success-200 bg-success-50 dark:border-success-500/30 dark:bg-success-500/10"}`}
              >
                <div>
                  ...
                  <p className="mt-1 text-xs text-default-500">
                    {remoteVersion.source === "release"
                      ? "正式版本"
                      : "版本 Tag"}
                    {remoteVersion.publishedAt
                      ? ` · ${new Date(remoteVersion.publishedAt).toLocaleDateString("zh-CN")}`
                      : ""}
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <a
                    href={remoteVersion.url}
                    rel="noopener noreferrer"
                    target="_blank"
                  >
          ...
          <Card className="border border-default-200 shadow-sm">
            <CardBody className="space-y-4 p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                {systemStatus && (
                  <p
                    className={`text-sm font-semibold ${updateStateClasses[systemStatus.state]}`}
                  >
                ...
                <div className="grid gap-3 text-sm sm:grid-cols-2">
                  <p className="text-default-500">
                    ...
                    <span className="text-foreground">
                      {formatTime(systemStatus.startedAt)}
                    </span>
                  </p>
                  <p className="text-default-500">
                    完成时间：
                    <span className="text-foreground">
                      {systemStatus.finishedAt
                        ? formatTime(systemStatus.finishedAt)
                        : "--"}
                    </span>
                  </p>
                </div>
