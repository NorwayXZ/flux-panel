...
const versionAtLeast = (version?: string, minimum = "2.32.0") => {
  ...
  return required.every(
    (part, index) =>
      ...
      current
        .slice(0, index)
        .some((value, position) => value > required[position]),
  );
};
...
const getDiscoveryStatusMeta = (status?: string) =>
  discoveryStatusMeta[
    status && status in discoveryStatusMeta
      ? (status as keyof typeof discoveryStatusMeta)
      : "disabled"
  ];
...
export default function HomeDevicesPage() {
  ...
  const [discoveryDevice, setDiscoveryDevice] =
    useState<InternalConnector | null>(null);
  const [discovery, setDiscovery] = useState<LanDiscoveryResult | null>(null);
  const [discoveryLoading, setDiscoveryLoading] = useState(false);
  const [scanLoading, setScanLoading] = useState(false);
  const [settingsLoading, setSettingsLoading] = useState(false);
  const [deleteDevice, setDeleteDevice] = useState<InternalConnector | null>(
    null,
  );
  ...
  const createDevice = async () => {
    ...
    setSubmitting(false);
    if (response.code !== 0)
      return toast.error(response.msg || "创建家庭设备失败");
    setCreateOpen(false);
  ...
  const openDiscovery = async (device: InternalConnector) => {
    ...
    if (response.code !== 0)
      return toast.error(response.msg || "读取服务发现结果失败");
  ...
  const toggleDiscovery = async (enabled: boolean) => {
    ...
    if (response.code !== 0)
      return toast.error(response.msg || "修改服务发现设置失败");
    ...
    setDevices((current) =>
      current.map((item) =>
        item.id === discoveryDevice.id
          ? {
              ...item,
              discoveryEnabled: enabled ? 1 : 0,
              discoveryStatus: enabled
                ? item.discoveryStatus === "disabled"
                  ? "idle"
  ...
  const runDiscovery = async () => {
    if (!discoveryDevice) return;
    setScanLoading(true);
    const response = await scanLanServices(
      discoveryDevice.id,
      scanCidr.trim() || "auto",
    );
    ...
    if (response.code !== 0) {
      setDiscovery((current) =>
        current
          ? { ...current, status: "failed", lastError: response.msg }
          : current,
      );
  ...
  const clearDiscovery = async () => {
    ...
    if (response.code !== 0)
      return toast.error(response.msg || "清空候选服务失败");
    setDiscovery((current) =>
  ...
  const confirmDeleteDevice = async () => {
    ...
    setDeleting(false);
    if (response.code !== 0)
      return toast.error(response.msg || "删除家庭设备失败");
    setDevices((current) =>
    ...
    <div className="mx-auto w-full max-w-[1500px] space-y-5 p-4 md:p-6">
      ...
      <header className="flex flex-col gap-4 border-b border-divider pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-semibold">家庭设备</h1>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {devices.map((device) => (
            <article
              key={device.id}
              className="rounded-md border border-divider bg-content1 p-5"
            >
              ...
              <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                <div>
                  <dt className="text-default-500">Agent 版本</dt>
                  <dd className="mt-1 font-medium">
                    {device.version || "尚未上报"}
                  </dd>
                </div>
                <div>
                  <dt className="text-default-500">最近连接</dt>
                  <dd className="mt-1 font-medium">
                    {formatTime(device.lastSeen)}
                  </dd>
                </div>
                <div>
                  <dt className="text-default-500">远端地址</dt>
                  <dd className="mt-1 break-all font-mono text-xs">
                    {device.remoteIp || "尚未上报"}
                  </dd>
                </div>
                <div>
                  <dt className="text-default-500">允许网段</dt>
                  <dd className="mt-1 break-all font-mono text-xs">
                    {device.allowedCidrs || "默认网段"}
                  </dd>
                </div>
              </dl>
              <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-divider pt-4">
                <Chip
                  color={getDiscoveryStatusMeta(device.discoveryStatus).color}
                  size="sm"
                  variant="flat"
                >
                  {device.discoveryEnabled
                    ? `${device.discoveredServiceCount || 0} 个候选服务`
                    : "服务发现已关闭"}
                </Chip>
                <div className="ml-auto flex flex-wrap justify-end gap-2">
                  ...
                  <Button
                    ...
                    variant="flat"
                    onPress={() =>
                      showCommand(device.id, device.platform, "install")
                    }
                  >
                    {device.online ? "安装命令" : "重新安装"}
                  </Button>
                  <Button
                    size="sm"
                    variant="flat"
                    onPress={() =>
                      showCommand(device.id, device.platform, "uninstall")
                    }
                  >
                    卸载命令
                  </Button>
                  <Tooltip content="删除设备记录">
                    <Button
                      isIconOnly
                      aria-label={`删除 ${device.name} 设备记录`}
                      color="danger"
                      size="sm"
                      variant="flat"
                      onPress={() => setDeleteDevice(device)}
                    >
                      <Trash2 size={15} />
                    </Button>
                  </Tooltip>
      ...
      <Modal isOpen={createOpen} size="xl" onOpenChange={setCreateOpen}>
        <ModalContent>
          <ModalHeader>添加家庭设备</ModalHeader>
          <ModalBody className="space-y-4">
            <Input
              label="设备名称"
              placeholder="家里 Windows 电脑"
              value={form.name}
            ...
            <Input
              label="允许访问的家庭网段（可选）"
              placeholder="留空使用本机和常见内网网段"
              value={form.allowedCidrs}
              onValueChange={(allowedCidrs) =>
                setForm({ ...form, allowedCidrs })
              }
            />
          </ModalBody>
          <ModalFooter>
      ...
      <Modal isOpen={commandOpen} size="2xl" onOpenChange={setCommandOpen}>
        <ModalContent>
          <ModalHeader>
            {commandAction === "install" ? "安装" : "卸载"}家庭 Agent
          </ModalHeader>
          <ModalBody className="space-y-4">
            <div className="flex flex-wrap gap-2">
              {(["linux", "windows", "macos"] as ConnectorPlatform[]).map(
                (platform) => (
                  <Button
                    key={platform}
                    color={commandPlatform === platform ? "primary" : "default"}
                    size="sm"
                    variant={commandPlatform === platform ? "solid" : "flat"}
                    onPress={() => switchCommand(platform, commandAction)}
                  >
              ...
              <div className="ml-auto flex gap-2">
                <Button
                  color={commandAction === "install" ? "primary" : "default"}
                  size="sm"
                  variant={commandAction === "install" ? "solid" : "flat"}
                  onPress={() => switchCommand(commandPlatform, "install")}
                >
                  安装
                </Button>
                <Button
                  color={commandAction === "uninstall" ? "danger" : "default"}
                  size="sm"
                  variant={commandAction === "uninstall" ? "solid" : "flat"}
                  onPress={() => switchCommand(commandPlatform, "uninstall")}
                >
                  卸载
                </Button>
              </div>
            </div>
            <div className="rounded-md bg-default-100 p-4">
              {commandLoading ? (
                <Spinner size="sm" />
              ) : (
          ...
          <ModalFooter>
            ...
            <Button
              color="primary"
              isDisabled={!command || commandLoading}
      ...
      <Modal
        ...
        size="lg"
        onOpenChange={(open) => !open && !deleting && setDeleteDevice(null)}
      >
        <ModalContent>
          <ModalHeader>删除家庭设备记录</ModalHeader>
          <ModalBody className="space-y-4">
            <div className="rounded-md border border-divider bg-default-50 px-4 py-3 text-sm">
              ...
              <div className="mt-1 text-default-500">
                Agent {deleteDevice?.version || "尚未上报"} ·{" "}
                {deleteDevice?.online ? "在线" : "离线"}
              </div>
            </div>
            <p className="text-sm text-default-600">
              此操作只删除面板记录，不会远程卸载
              Agent。设备必须离线，并且不能被内网映射或家庭网络中转使用。
            </p>
            {deleteDevice?.online && (
              <div className="rounded-md border border-danger/30 bg-danger/10 px-4 py-3 text-sm text-danger">
                设备仍然在线。请先执行卸载命令，等待设备离线后再删除记录。
              </div>
            )}
          </ModalBody>
          <ModalFooter>
            <Button
              isDisabled={deleting}
              variant="flat"
              onPress={() => setDeleteDevice(null)}
            >
              取消
            </Button>
            <Button
              color="danger"
              isDisabled={Boolean(deleteDevice?.online)}
              isLoading={deleting}
      ...
      >
        <ModalContent>
          <ModalHeader>
            {discoveryDevice?.name || "家庭设备"} · 局域网服务发现
          </ModalHeader>
              ...
              <>
                <section className="grid gap-px overflow-hidden rounded-md border border-divider bg-divider sm:grid-cols-3">
                  <div className="bg-content1 px-4 py-3">
                    <div className="text-xs text-default-500">发现状态</div>
                    <div className="mt-2">
                      <Chip
                        color={getDiscoveryStatusMeta(discovery.status).color}
                        size="sm"
                        variant="flat"
                      >
                        {getDiscoveryStatusMeta(discovery.status).label}
                      </Chip>
                    </div>
                  </div>
                  <div className="bg-content1 px-4 py-3">
                    <div className="text-xs text-default-500">候选服务</div>
                    <div className="mt-1 text-xl font-semibold">
                      {discovery.services.length}
                    </div>
                  </div>
                  <div className="bg-content1 px-4 py-3">
                    <div className="text-xs text-default-500">最近扫描</div>
                    <div className="mt-1 text-sm font-medium">
                      {formatTime(discovery.lastScanAt)}
                    </div>
                  </div>
                </section>
                ...
                <section className="grid gap-4 border-y border-divider py-5 lg:grid-cols-[minmax(0,1fr)_220px] lg:items-end">
                  <div className="space-y-4">
                    <Switch
                      ...
                      isSelected={discovery.enabled}
                      onValueChange={toggleDiscovery}
                    >
                      启用局域网服务发现
                    </Switch>
                    <Input
                      description="仅允许当前家庭设备授权范围内的 IPv4 私网；单次范围不能大于 /24。"
                      label="扫描网段"
                      placeholder="自动识别，或填写 192.168.100.0/24"
                      startContent={
                        <Network className="text-default-400" size={16} />
                      }
                      value={scanCidr}
                      onValueChange={setScanCidr}
                    />
                  </div>
                  <Button
                    color="primary"
                    isDisabled={
                      !discovery.enabled ||
                      !discoveryDevice.online ||
                      !versionAtLeast(discoveryDevice.version)
                    }
                    isLoading={scanLoading}
                    startContent={<Radar size={17} />}
                ...
                {!discoveryDevice.online && (
                  <div className="rounded-md border border-warning/30 bg-warning/10 px-4 py-3 text-sm text-warning-700 dark:text-warning-400">
                    家庭设备当前离线，恢复在线后才能扫描。
                  </div>
                )}
                {discoveryDevice.online &&
                ...
                <section>
                  <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <h3 className="text-sm font-semibold">发现结果</h3>
                      {discovery.lastCidr && (
                        <p className="mt-1 text-xs text-default-500">
                          扫描范围：{discovery.lastCidr}
                        </p>
                      )}
                    </div>
                    <Button
                      color="danger"
                      isDisabled={discovery.services.length === 0}
                      size="sm"
                  ...
                  {discovery.services.length === 0 ? (
                    <div className="flex min-h-36 flex-col items-center justify-center gap-2 border-y border-divider text-default-500">
                      <Radar size={26} />
                      <span className="text-sm">暂无候选服务</span>
                    </div>
                  ) : (
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                {service.sensitive === 1 && (
                                  <Chip
                                    color="warning"
                                    size="sm"
                                    startContent={<ShieldAlert size={13} />}
                                    variant="flat"
                                  >
                                    敏感服务
                                  </Chip>
                              ...
                              <p className="mt-1 truncate font-mono text-sm text-default-600">
                          ...
                          <div className="mt-3 min-h-5 truncate text-xs text-default-500">
                            {service.product || service.serviceName}
                          </div>
          ...
          <ModalFooter>
            <Button variant="flat" onPress={() => setDiscoveryOpen(false)}>
              关闭
            </Button>
          </ModalFooter>
        </ModalContent>
