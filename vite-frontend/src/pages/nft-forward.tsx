...
const protocolLabel = (protocol: string) =>
  protocol === "tcp_udp" ? "TCP + UDP" : protocol.toUpperCase();
...
export default function NftForwardPage() {
  ...
  const load = async (quiet = false) => {
    ...
    if (response.code !== 0)
      return toast.error(response.msg || "读取 nftables 转发失败");
  ...
  const toggle = async (rule: NftForwardRule) => {
    ...
    if (!enabled && !window.confirm(`暂停“${rule.name}”？新连接将不再转发。`))
      return;
    setBusy(rule.id);
  ...
  const rollback = async (rule: NftForwardRule) => {
    if (
      !window.confirm(`将“${rule.name}”回退到上次成功配置？当前配置会被替换。`)
    )
      return;
    setBusy(rule.id);
  ...
  const remove = async (rule: NftForwardRule) => {
    if (
      !window.confirm(
        `删除“${rule.name}”？Agent 确认规则移除前，端口不会释放。`,
      )
    )
      return;
    setBusy(rule.id);
  ...
  return (
    <div className="mx-auto w-full max-w-[1680px] space-y-5 p-4 md:p-6">
      <header className="flex flex-col gap-4 border-b border-divider pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm text-default-500">内核网络</p>
          <h1 className="mt-1 text-2xl font-semibold">nftables 端口转发</h1>
        </div>
        <div className="flex gap-2">
          <Tooltip content="刷新规则和计数器">
            <Button
              isIconOnly
              aria-label="刷新"
              variant="flat"
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        {[
          ["规则", data.summary.total],
          ["运行中", data.summary.active],
          ["已暂停", data.summary.paused],
          ["异常", data.summary.errors],
          ["当前计数", formatBytes(data.summary.bytes)],
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="mt-1 leading-6 text-default-500">
              只管理 Linux 节点中的 <code>table ip cloudnest_nat</code>
              。创建前同时检查面板端口账本、服务器真实监听和外部
              DNAT；每次变更先校验再原子应用，失败时 Agent 保留上一份成功规则。
            </p>
          </div>
        </div>
      </section>
      ...
      {loading ? (
        ...
        <section className="overflow-hidden rounded-md border border-divider bg-content1">
          ...
          {rules.map((rule, index) => (
            <div
              key={rule.id}
              className={`grid gap-3 px-4 py-4 xl:grid-cols-[minmax(150px,1fr)_minmax(190px,1.2fr)_32px_minmax(180px,1fr)_130px_150px_190px] xl:items-center ${index ? "border-t border-divider" : ""}`}
            >
              <div className="min-w-0">
                <div className="truncate font-medium">{rule.name}</div>
                <p className="mt-1 truncate text-xs text-default-500">
                  {rule.nodeName} · Agent {rule.agentVersion || "未知"}
                </p>
              </div>
              <div className="min-w-0">
                <p className="truncate font-mono text-sm">
                  {rule.publicHost || rule.listenAddress}:{rule.listenPort}
                </p>
                <p className="mt-1 text-xs text-default-500">
                  监听 {rule.listenAddress} · {protocolLabel(rule.protocol)}
                </p>
              </div>
              <ArrowRight
                className="hidden text-default-400 xl:block"
                size={18}
              />
              <div className="min-w-0">
                <p className="truncate font-mono text-sm">
                ...
                <p className="mt-1 truncate text-xs text-default-500">
                  {rule.sourceCidrs
                    ? `仅允许 ${rule.sourceCidrs.split(/\s+/).length} 个来源网段`
                    : "允许任意来源"}
                </p>
              </div>
              <div>
                <Chip size="sm" variant="flat">
                  {rule.natMode === "masquerade" ? "标准 NAT" : "保留来源 IP"}
                </Chip>
              </div>
              <div>
                <Chip color={stateColor(rule.state)} size="sm" variant="flat">
                  {stateLabel[rule.state] || rule.state}
                </Chip>
                <p className="mt-1 text-xs text-default-500">
                  {formatBytes(rule.byteCount)} · {rule.packetCount || 0} 包
                </p>
              </div>
              <div className="flex justify-end gap-1">
                <Tooltip content="检查状态">
                  <Button
                    isIconOnly
                    aria-label="检查状态"
                ...
                <Tooltip content={rule.enabled ? "暂停" : "恢复"}>
                  <Button
                    isIconOnly
                    aria-label={rule.enabled ? "暂停" : "恢复"}
                ...
                <Tooltip content="编辑">
                  <Button
                    ...
                    size="sm"
                    variant="light"
                    onPress={() => openForm(rule)}
                  >
                    <Pencil size={16} />
                  </Button>
                </Tooltip>
                <Tooltip content="事件记录">
                  <Button
                    isIconOnly
                    aria-label="事件记录"
                    size="sm"
                    variant="light"
                    onPress={() => showEvents(rule)}
                  >
                    <History size={16} />
                  </Button>
                </Tooltip>
                {rule.rollbackAvailable && (
                  <Tooltip content="回退到上次成功配置">
                    <Button
                      isIconOnly
                      aria-label="回退"
                      color="warning"
              ...
              {(rule.lastError || rule.lastWarning) && (
                <div className="xl:col-span-7 border-t border-divider pt-3 text-xs">
                  {rule.lastError && (
                    <p className="flex gap-2 text-danger">
                      <TriangleAlert className="shrink-0" size={14} />
                      {rule.lastError}
                    </p>
                  )}
                  {rule.lastWarning && (
      ...
      >
        <ModalContent>
          <ModalHeader>
            {form.id ? "编辑 nftables 转发" : "新建 nftables 转发"}
          </ModalHeader>
          <ModalBody className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Select
                isRequired
                description={`Linux Agent ${data.minimumAgentVersion} 或更高版本`}
            ...
            <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_150px_180px]">
              <Input
                description="0.0.0.0 表示监听节点全部 IPv4"
                label="监听 IPv4"
                value={form.listenAddress}
                onValueChange={(value) => {
                  setForm({ ...form, listenAddress: value });
                  setPreflight(null);
                }}
              />
              <Input
            ...
            <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_150px]">
              <Input
                isRequired
                description="第一版只接受固定 IPv4，不解析域名"
                label="目标 IPv4"
                placeholder="10.0.0.8"
                value={form.targetAddress}
                onValueChange={(value) => {
                  setForm({ ...form, targetAddress: value });
                  setPreflight(null);
                }}
              />
              <Input
            ...
            <Select
              label="NAT 模式"
              selectedKeys={[form.natMode]}
              onSelectionChange={(keys) => {
                setForm({
                  ...form,
                  natMode: String(
                    Array.from(keys)[0] || "masquerade",
                  ) as NftForwardNatMode,
                });
                setPreflight(null);
              }}
            >
              <SelectItem key="masquerade" textValue="标准 NAT">
                标准 NAT · 目标无需配置返回路由
              </SelectItem>
              <SelectItem key="preserve_source" textValue="保留来源 IP">
                保留来源 IP · 目标必须经本节点返回流量
              </SelectItem>
            </Select>
            <Textarea
              description="留空允许任意来源；最多 64 个 CIDR，可用换行、逗号或空格分隔"
              label="来源 IPv4 白名单"
              minRows={3}
              placeholder={"203.0.113.8/32\n198.51.100.0/24"}
              value={form.sourceCidrs}
              onValueChange={(value) => {
                setForm({ ...form, sourceCidrs: value });
                setPreflight(null);
              }}
            />
            <Switch
              isSelected={form.enabled}
              onValueChange={(value) => setForm({ ...form, enabled: value })}
            >
              保存后立即启用
            </Switch>
            {form.natMode === "preserve_source" && (
              <div className="border-y border-warning-200 bg-warning-50 px-3 py-3 text-sm text-warning-700 dark:bg-warning-900/10 dark:text-warning-300">
                保留来源 IP 不做
                SNAT。目标主机的默认网关或静态路由必须把回程交回此节点，否则连接只会收到单向数据。
              </div>
            )}
            {preflight && (
              <div className="border-y border-divider py-3 text-sm">
                <p className="flex items-center gap-2 font-medium text-success">
                  <CircleCheck size={16} />
                  预检通过 · {preflight.nftVersion || "nftables 可用"}
                </p>
                <p className="mt-1 text-default-500">
                  IPv4 转发{" "}
                  {preflight.ipv4Forwarding ? "已启用" : "将在应用时启用"}
                  {preflight.firewallManager
          ...
          <ModalFooter>
            ...
            <Button color="primary" isLoading={busy === "save"} onPress={save}>
              保存并应用
            </Button>
          </ModalFooter>
        </ModalContent>
      ...
      >
        <ModalContent>
          <ModalHeader>{eventRule?.name || "规则"} · 事件记录</ModalHeader>
          <ModalBody>
            {events.length === 0 ? (
              ...
              <div className="divide-y divide-divider border-y border-divider">
                {events.map((event) => (
                  <div
                    key={event.id}
                    className="grid gap-2 py-3 sm:grid-cols-[110px_110px_minmax(0,1fr)_170px] sm:items-center"
                  >
                    <Chip
                      className="w-fit"
                      color={
          ...
          <ModalFooter>
            <Button variant="flat" onPress={() => setEventsOpen(false)}>
              关闭
            </Button>
          </ModalFooter>
        </ModalContent>
