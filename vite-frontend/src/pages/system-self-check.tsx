...
export default function SystemSelfCheckPage() {
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    if (response.code !== 0)
      return toast.error(response.msg || "读取自检结果失败");
  ...
  const start = async () => {
    setRunning(true);
    const nodeId = scopeNodeId.startsWith("node:")
      ? Number(scopeNodeId.slice(5))
      : undefined;
    const connectorId = scopeNodeId.startsWith("connector:")
      ? Number(scopeNodeId.slice(10))
      : undefined;
  ...
  const categories = useMemo(
    () =>
      Array.from(new Set((data?.findings || []).map((item) => item.category))),
    [data?.findings],
  );
  const scopeOptions = useMemo(
    () => [
      ...
      ...(data?.nodes || []).map((node) => ({
        key: `node:${node.id}`,
        label: `服务器 · ${node.name} · ${node.status === 1 ? "在线" : "离线"} · Agent ${node.version || "未知"}`,
      })),
      ...(data?.connectors || []).map((connector) => ({
        key: `connector:${connector.id}`,
        label: `接入设备 · ${connector.name} · ${connector.online ? "在线" : "离线"} · ${connector.platform} · Agent ${connector.version || "未知"}`,
      })),
    ],
    ...
    <div className="mx-auto w-full max-w-[1500px] space-y-5 p-4 md:p-6">
      <header className="flex flex-col gap-4 border-b border-divider pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-semibold">全系统自检中心</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-default-500">
            服务器节点由 Agent 检查自身配置；Windows、macOS、Linux 接入设备由
            Connector 检查本机
            DNS、IPv4/IPv6、默认路由，以及到面板和已登记入口端口的实际 TCP
            可达性。所有检查均为只读。
          </p>
        </div>
        <div className="grid w-full gap-3 sm:grid-cols-[260px_auto] lg:w-auto">
          <Select
            aria-label="自检范围"
            isDisabled={running}
            selectedKeys={[scopeNodeId]}
            onSelectionChange={(keys) =>
              setScopeNodeId(String(Array.from(keys)[0] || "all"))
            }
          >
          ...
          <Button
            color="primary"
            isLoading={running || loading}
            startContent={!running && <SearchCheck size={17} />}
            onPress={() => void start()}
          >
            {running ? "自检运行中" : "开始自检"}
          </Button>
      ...
      <section className="grid grid-cols-2 border-y border-divider sm:grid-cols-5">
        {[
          ...
          <div
            key={String(label)}
            className={`px-4 py-4 ${index > 0 ? "border-l border-divider" : ""}`}
          >
      ...
      {run && (
        <section className="flex flex-col gap-3 border-b border-divider pb-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium">{run.message || "等待自检"}</p>
              <p className="mt-1 text-xs text-default-500">
                {new Date(run.startedAt).toLocaleString("zh-CN")} ·{" "}
                {run.scopeType === "connector"
                  ? "指定接入设备"
                  : run.scopeType === "node" || run.scopeNodeId
                    ? "指定服务器节点"
                    : "全部资源"}
              </p>
            </div>
          ...
          <p className="text-xs text-default-500">
            服务器 Agent 自检最低版本 {data?.minimumAgentVersion} · Connector
            本机自检最低版本 {data?.minimumConnectorVersion || "2.41.4"}
          </p>
        </section>
      )}
      ...
      <section className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="grid grid-cols-2 gap-2 sm:w-[390px]">
          ...
          >
            ...
            <SelectItem key="failed">仅故障</SelectItem>
            <SelectItem key="warning">仅注意</SelectItem>
            <SelectItem key="healthy">仅正常</SelectItem>
            <SelectItem key="skipped">仅跳过</SelectItem>
          </Select>
          <Select
            aria-label="检查类别"
      ...
      <section className="border-t border-divider">
        {findings.map((finding) => (
          <article
            key={finding.id}
            className="grid gap-4 border-b border-divider py-4 lg:grid-cols-[190px_minmax(0,1fr)_minmax(240px,0.75fr)]"
          >
            <div className="flex items-start gap-3">
              <StatusIcon status={finding.status} />
              <div className="min-w-0">
                <Chip
                  color={statusMeta[finding.status].color}
                  size="sm"
                  variant="flat"
                >
            ...
            <div className="min-w-0">
              <p className="text-xs text-default-500">{finding.faultSegment}</p>
              <h3 className="mt-1 font-medium">{finding.summary}</h3>
              {finding.evidence && (
                <p className="mt-2 break-words text-sm leading-6 text-default-600">
                  证据：{finding.evidence}
                </p>
              )}
              {finding.impact && finding.impact !== "无" && (
                <p className="mt-1 text-sm leading-6 text-default-500">
                  影响：{finding.impact}
                </p>
              )}
            </div>
            <div className="border-l-0 border-divider lg:border-l lg:pl-4">
              <p className="text-xs text-default-500">
                处理建议 ·{" "}
                {resourceLabels[finding.resourceType] || finding.resourceType}
              </p>
              <p className="mt-1 text-sm leading-6">
                {finding.remediation || "无需操作"}
              </p>
              {finding.category === "agent" &&
                finding.resourceType === "node" &&
                finding.status === "warning" &&
                finding.summary.includes("不同机器") &&
                finding.resourceId && (
                  <Button
                    className="mt-3"
                    isLoading={resettingNode === finding.resourceId}
                    size="sm"
                    startContent={<RotateCcw size={15} />}
                    variant="flat"
                    onPress={() => void resetBaseline(finding)}
                  >
                    重置身份基线
                  </Button>
                )}
...
function StatusIcon({ status }: { status: SelfCheckStatus }) {
  if (status === "healthy")
    return <CircleCheck className="mt-0.5 shrink-0 text-success" size={20} />;
  if (status === "warning")
    return <TriangleAlert className="mt-0.5 shrink-0 text-warning" size={20} />;
  if (status === "failed")
    return <XCircle className="mt-0.5 shrink-0 text-danger" size={20} />;

  return (
    <CircleDashed className="mt-0.5 shrink-0 text-default-400" size={20} />
  );
}
