...
const initialTemplateForm = { name: "", sourceGroupId: "", startPort: "10000", endPort: "20000", protocolMode: "tcp", blockedTargetCidrs: "", blockPlatformNodes: true };
...
export default function AuthorizedEntryPage() {
  ...
  const saveTemplate = async () => {
    setSubmitting(true);
    const result = await saveAuthorizedEntryTemplate({ ...templateForm, blockPlatformNodes: templateForm.blockPlatformNodes, sourceGroupId: Number(templateForm.sourceGroupId), startPort: Number(templateForm.startPort), endPort: Number(templateForm.endPort) });
    setSubmitting(false);
  ...
  return <div className="space-y-6">
    ...
    {loading ? <p className="py-12 text-center text-default-500">正在加载授权链路…</p> : grants.length === 0 ? <div className="border-y border-divider py-16 text-center text-default-500">暂无授权入口</div> : <section className="grid items-start gap-4 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">{grants.map(grant => {
      ...
      return <Card key={grant.id} radius="sm" shadow="none" className="border border-divider"><CardHeader className="flex justify-between gap-3"><div className="min-w-0"><h2 className="truncate font-semibold">{grant.name}</h2><p className="mt-1 truncate text-sm text-default-500">{grant.accessHost}</p></div><div className="flex items-center gap-1"><Chip color={stateColor(grant.state) as any} size="sm" variant="flat">{grantLabel(grant)}</Chip>{admin && <><Button isIconOnly aria-label="编辑授权" size="sm" variant="light" onPress={() => openGrantEditor(grant)}><Pencil size={15} /></Button><Button isIconOnly aria-label={grant.state === "active" ? "暂停授权" : "恢复授权"} size="sm" variant="light" onPress={() => void changeGrantState(grant, grant.state !== "active")}>{grant.state === "active" ? <Pause size={15} /> : <Play size={15} />}</Button><Button isIconOnly aria-label="撤销授权" color="danger" size="sm" variant="light" onPress={() => void revokeGrant(grant)}><Trash2 size={15} /></Button></>}</div></CardHeader><CardBody className="gap-4 pt-0">
        ...
        {grant.state === "active" && grant.ports.length === 0 && <p className="border-t border-divider pt-3 text-xs text-default-500">授权已发放，等待用户添加落地；当前不占用实际转发端口，也不会产生流量。</p>}
