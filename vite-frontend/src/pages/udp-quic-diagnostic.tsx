...
const initial: UdpQuicDiagnosticOverview = {
  minimumAgentVersion: "2.49.0",
  nodes: [],
  tasks: [],
  summary: { total: 0, running: 0, success: 0, degraded: 0, failed: 0 },
};
...
const latencyText = (value?: number) =>
  value && value > 0 ? `${Number(value).toFixed(2)} ms` : "-";
const modeText = (mode: string) => (mode === "quic" ? "QUIC 握手" : "UDP Echo");
const stateMeta = (task: UdpQuicDiagnosticTask) =>
...
export default function UdpQuicDiagnosticPage() {
  ...
  const [historyTask, setHistoryTask] = useState<UdpQuicDiagnosticTask | null>(
    null,
  );
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    else if (!quiet) toast.error(response.msg || "加载 UDP / QUIC 诊断失败");
  }, []);
  ...
  const agentNodeDisabledKeys = useMemo(
    () =>
      data.nodes
        .filter((node) => node.status !== 1 || !node.compatible)
  ...
  const noCompatibleNodes =
    !loading &&
    onlineNodes.length > 0 &&
    onlineNodes.every((node) => !node.compatible);
  const targetNodeOptions = form.mode === "udp_echo" ? data.nodes : onlineNodes;
  const nodeOptionText = (node: (typeof data.nodes)[number], address = false) =>
    `${node.name} · ${address ? `${node.serverIp || node.ip || "未设置地址"} · ` : ""}${node.status !== 1 ? "离线" : !node.compatible ? `需升级到 ${data.minimumAgentVersion}` : "可用"} · Agent ${node.version || "未知"}`;
  const agentBlockReason = (nodeId: number | undefined, label: string) => {
    ...
    if (!node.compatible)
      return `${label}节点 Agent 需要升级到 ${data.minimumAgentVersion}`;
  ...
  const taskRunBlockReason = (task: UdpQuicDiagnosticTask) => {
    ...
    return task.mode === "udp_echo"
      ? agentBlockReason(task.targetNodeId, "目标")
      : "";
  };
  ...
  const openEdit = (task: UdpQuicDiagnosticTask) => {
    setForm({
      ...
      targetType: task.targetType || "node",
      targetNodeId: task.targetNodeId ? String(task.targetNodeId) : "",
      targetHost: task.targetHost || "",
      port: String(task.port),
      mode: task.mode || "udp_echo",
      serverName: task.serverName || "",
      ipFamily: task.ipFamily || "auto",
      sampleCount: String(task.sampleCount),
      timeoutMs: String(task.timeoutMs),
      packetSize: String(task.packetSize),
      idleTimeoutSeconds: String(task.idleTimeoutSeconds),
      alpn: task.alpn || "h3",
      verifyCertificate: bool(task.verifyCertificate),
  ...
  const input = (): UdpQuicDiagnosticInput => ({
    ...
    targetType: form.mode === "udp_echo" ? "node" : form.targetType,
    targetNodeId:
      form.targetType === "node" || form.mode === "udp_echo"
        ? Number(form.targetNodeId)
        : undefined,
    targetHost:
      form.targetType === "custom" && form.mode === "quic"
    ...
    idleTimeoutSeconds:
      form.mode === "udp_echo" ? Number(form.idleTimeoutSeconds) : 0,
    alpn: form.mode === "quic" ? form.alpn.trim() || "h3" : undefined,
    verifyCertificate: form.verifyCertificate,
    retentionDays: Number(form.retentionDays),
  });
  const save = async () => {
    if (!form.name.trim() || !form.sourceNodeId)
      return toast.error("请填写任务名称并选择执行节点");
    if (
      (form.mode === "udp_echo" || form.targetType === "node") &&
      !form.targetNodeId
    )
      return toast.error("请选择目标节点");
    if (
      form.mode === "quic" &&
      form.targetType === "custom" &&
      !form.targetHost.trim()
    )
      return toast.error("请填写 QUIC 目标地址");
    ...
    <div className="mx-auto w-full max-w-[1500px] space-y-6 p-4 sm:p-6">
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        {[
          ...
          [
            "正常",
            loading ? "-" : data.summary.success,
            <Waves size={18} />,
            "text-success bg-success/10",
          ],
          [
            "不稳定",
            loading ? "-" : data.summary.degraded,
            <History size={18} />,
            "text-warning bg-warning/10",
          ],
          [
            "失败",
            loading ? "-" : data.summary.failed,
            <Trash2 size={18} />,
      ...
      {noCompatibleNodes && (
        <section className="border border-warning-200 bg-warning-50 p-4 text-sm text-warning-700 dark:border-warning-800 dark:bg-warning-900/20 dark:text-warning-300">
          当前在线节点 Agent 均低于 {data.minimumAgentVersion}，UDP / QUIC
          诊断需要先升级 Agent 后才能执行。
        </section>
      )}
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Chip size="sm" variant="flat">
                        {modeText(task.mode)} · {task.ipFamily.toUpperCase()}
                      </Chip>
                      {blockReason && (
                        <Chip
                          color="warning"
                    ...
                    <p className="mt-2 truncate text-sm text-default-500">
                      {task.sourceNodeName} →{" "}
                      {task.targetType === "node"
                        ? task.targetNodeName
                        : task.targetHost}
                      :{task.port}
                    </p>
                    <p className="mt-1 text-xs text-default-400">
                      Agent {data.minimumAgentVersion}+ · {task.sampleCount} 次
                      · {task.timeoutMs} ms
                    </p>
                  </div>
                  <div className="flex gap-1">
                    <Button
                      isIconOnly
                      isDisabled={bool(task.running) || Boolean(blockReason)}
                      isLoading={busy === `run-${task.id}`}
                      size="sm"
                      title={blockReason || "开始诊断"}
                      variant="light"
                ...
                <div className="mt-4 grid grid-cols-4 divide-x divide-divider border-y border-divider py-3 text-center">
                  <div>
                    ...
                    <p className="mt-1 font-semibold">
                      ...
                      {Number(task.latestSampleCount || task.sampleCount || 0)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-default-500">丢包</p>
                    <p className="mt-1 font-semibold">
                      {percentText(task.packetLossPercent ?? task.failureRate)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-default-500">
                      {task.mode === "quic" ? "握手" : "RTT"}
                    </p>
                    <p className="mt-1 font-semibold">
                      {latencyText(task.quicHandshakeAvgMs || task.rttAvgMs)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-default-500">抖动</p>
                    <p className="mt-1 font-semibold">
                ...
                <div className="mt-3 flex items-start justify-between gap-3 text-xs text-default-500">
                  ...
                  {task.diagnosis || task.lastError ? (
                    <span
                      className={
                        task.lastStatus === "success"
                          ? "max-w-[68%] truncate"
                          : "max-w-[68%] truncate text-warning-600 dark:text-warning-400"
                      }
                      title={task.diagnosis || task.lastError}
                    >
                      {task.diagnosis || task.lastError}
                    </span>
                  ) : null}
      ...
      >
        <ModalContent>
          <ModalHeader>
            {form.id ? "编辑 UDP / QUIC 诊断" : "新建 UDP / QUIC 诊断"}
          </ModalHeader>
          <ModalBody className="space-y-4">
            <Input
              isRequired
              label="任务名称"
              placeholder="移动 5G 到香港 QUIC"
              value={form.name}
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <Select
                ...
                onSelectionChange={(keys) => {
                  const mode = String(
                    Array.from(keys)[0] || "udp_echo",
                  ) as FormState["mode"];

                  setForm({
                    ...form,
                    mode,
                    targetType: mode === "udp_echo" ? "node" : form.targetType,
                    port: mode === "quic" ? form.port : form.port,
                  });
                }}
              >
                <SelectItem key="udp_echo">UDP Echo</SelectItem>
                <SelectItem key="quic">QUIC 握手</SelectItem>
              </Select>
              <Select
                isRequired
                disabledKeys={agentNodeDisabledKeys}
                label="执行节点"
            ...
            {form.mode === "quic" && (
              ...
              >
                <SelectItem key="custom">自定义目标</SelectItem>
                <SelectItem key="node">节点地址</SelectItem>
              </Select>
            )}
            {form.mode === "udp_echo" || form.targetType === "node" ? (
              <Select
                isRequired
                disabledKeys={
                  form.mode === "udp_echo" ? agentNodeDisabledKeys : []
                }
                label="目标节点"
            ...
            {form.mode === "udp_echo" ? (
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Input
                  ...
                  onValueChange={(value) =>
                    setForm({ ...form, idleTimeoutSeconds: value })
                  }
                />
              </div>
            ) : (
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Input
                  ...
                  placeholder="h3"
                  value={form.alpn}
                  onValueChange={(value) => setForm({ ...form, alpn: value })}
                />
                <div className="md:col-span-2">
                  <Switch
                    isSelected={form.verifyCertificate}
                    onValueChange={(value) =>
                      setForm({ ...form, verifyCertificate: value })
                    }
                  >
                    校验证书
                  </Switch>
      ...
      >
        <ModalContent>
          <ModalHeader>{historyTask?.name} · 诊断历史</ModalHeader>
          <ModalBody>
            {runs.length === 0 ? (
                ...
                <table className="w-full min-w-[1180px] text-left text-sm">
                  <thead className="border-b border-divider text-xs text-default-500">
                    <tr>
                      ...
                      <th className="p-3">类型 / 目标</th>
                      <th className="p-3">成功</th>
                      <th className="p-3">丢包</th>
                      <th className="p-3">RTT / 握手</th>
                      <th className="p-3">抖动</th>
                  ...
                  <tbody>
                    {runs.map((run) => (
                      <tr key={run.id} className="border-b border-divider/60">
                        ...
                        <td className="p-3">
                          <div>
                            {modeText(run.mode)} · {run.ipFamily}
                          </div>
                          <div className="mt-1 max-w-[260px] truncate text-xs text-default-400">
                            {run.targetHost}:{run.port}
                            {run.resolvedAddress
                              ? ` · ${run.resolvedAddress}`
                              : ""}
                          </div>
                        ...
                        <td className="p-3">
                          {run.successCount}/{run.sampleCount}
                        </td>
                        <td className="p-3">
                          {percentText(
                            run.packetLossPercent || run.failureRate,
                          )}
                        </td>
                        <td className="p-3">
                          {latencyText(run.quicHandshakeAvgMs || run.rttAvgMs)}
                        </td>
                        <td className="p-3">{latencyText(run.jitterMs)}</td>
                        <td className="p-3">
                          {run.natIdleSeconds
                            ? `${run.natIdleSeconds}s · ${bool(run.natIdleAlive) ? "保持" : "失效"}`
                            : "-"}
                        </td>
                        <td className="p-3">
                          <span
                            className="line-clamp-2 max-w-[320px]"
                            title={run.diagnosis || run.error}
                          >
                            {run.diagnosis || run.error || "-"}
                          </span>
                        </td>
                        <td className="p-3">
                          <Chip
