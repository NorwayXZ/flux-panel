...
type FormState = { id?: number; name: string; sourceNodeId: string; targetNodeId: string; listenPort: string; direction: BandwidthTestTask['direction']; streams: string; durationSeconds: string; maximumMegabytes: string; retentionDays: string };
const emptyForm: FormState = { name: '', sourceNodeId: '', targetNodeId: '', listenPort: '5201', direction: 'bidirectional', streams: '4', durationSeconds: '10', maximumMegabytes: '512', retentionDays: '30' };
const initial: BandwidthTestOverview = { minimumAgentVersion: '2.44.0', nodes: [], tasks: [], summary: { total: 0, running: 0, success: 0, failed: 0, peakMbps: 0 } };
...
const bytes = (value?: number) => { if (!value) return '-'; const units = ['B', 'KB', 'MB', 'GB']; let size = value; let unit = 0; while (size >= 1024 && unit < units.length - 1) { size /= 1024; unit++; } return `${size.toFixed(unit > 1 ? 2 : 0)} ${units[unit]}`; };
...
export default function BandwidthTestPage() {
  ...
  const openEdit = (task: BandwidthTestTask) => { setForm({ id: task.id, name: task.name, sourceNodeId: String(task.sourceNodeId), targetNodeId: String(task.targetNodeId), listenPort: String(task.listenPort), direction: task.direction, streams: String(task.streams), durationSeconds: String(task.durationSeconds), maximumMegabytes: String(task.maximumMegabytes), retentionDays: String(task.retentionDays) }); setFormOpen(true); };
  const input = (): BandwidthTestTaskInput => ({ id: form.id, name: form.name.trim(), sourceNodeId: Number(form.sourceNodeId), targetNodeId: Number(form.targetNodeId), listenPort: Number(form.listenPort), direction: form.direction, streams: Number(form.streams), durationSeconds: Number(form.durationSeconds), maximumMegabytes: Number(form.maximumMegabytes), retentionDays: Number(form.retentionDays) });
  const save = async () => {
