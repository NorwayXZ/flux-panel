...
const auditStatus: Record<string, string> = {
  connecting: '连接中', active: '已连接', closed: '已结束', failed: '失败',
};
...
export default function NodeTerminalPage() {
  ...
  const supported = useMemo(() => versionAtLeast(node?.version), [node?.version]);

  const loadPage = useCallback(async () => {
    setLoading(true);
    const [nodeResult, auditResult] = await Promise.all([getNodeList(), getTerminalAudit(numericNodeId)]);
    if (nodeResult.code === 0) {
      setNode((nodeResult.data || []).find((item: TerminalNode) => item.id === numericNodeId) || null);
    }
  ...
  const ensureTerminal = () => {
    ...
    const terminal = new Terminal({
      ...
      scrollback: 5000,
      theme: dark
        ? { background: '#0b0d10', foreground: '#e5e7eb', cursor: '#60a5fa', selectionBackground: '#334155' }
        : { background: '#111318', foreground: '#f3f4f6', cursor: '#60a5fa', selectionBackground: '#334155' },
    });
  ...
  const connectSocket = (ticket: string) => {
    ...
    const socket = new WebSocket(`${wsBase}/terminal?ticket=${encodeURIComponent(ticket)}`);
    ...
    socket.onopen = () => {
      ...
      if (terminalContainerRef.current) resizeObserverRef.current.observe(terminalContainerRef.current);
      sendResize();
    };
    ...
    socket.onmessage = event => {
      ...
      if (message.type === 'ready') {
        ...
        closeSocket(false);
        getTerminalAudit(numericNodeId).then(result => result.code === 0 && setAudits(result.data || []));
      }
    };
    ...
    socket.onclose = () => {
      ...
      setConnecting(false);
      socketRef.current = null;
      setSessionStatus(current => current.startsWith('已连接') ? '连接已断开' : current);
    };
  };
  ...
  if (!node) return <div className="p-6 text-default-500">节点不存在或无权访问</div>;
    ...
    <div className="mx-auto w-full max-w-[1680px] space-y-5 p-4 md:p-6">
      <header className="flex flex-col gap-4 border-b border-divider pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="flex min-w-0 items-center gap-3">
          <Button isIconOnly variant="light" aria-label="返回节点列表" onPress={() => navigate('/node')}><ArrowLeft size={20} /></Button>
          <div className="min-w-0">
            <p className="text-sm text-default-500">节点远程终端</p>
            <h1 className="mt-1 truncate text-2xl font-semibold">{node.name}</h1>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Chip size="sm" variant="flat" color={node.status === 1 ? 'success' : 'danger'}>{node.status === 1 ? '节点在线' : '节点离线'}</Chip>
          <Chip size="sm" variant="flat" color={supported ? 'primary' : 'warning'}>Agent {node.version || '未知'}</Chip>
          <Chip size="sm" variant="flat" color={node.terminalEnabled ? 'success' : 'default'}>{node.terminalEnabled ? '终端已启用' : '终端已关闭'}</Chip>
        </div>
      </header>
      ...
      {!supported && (
        <section className="flex items-start gap-3 border-y border-warning/40 bg-warning/10 px-4 py-4 text-sm text-warning-700 dark:text-warning-300">
          ...
          <div><div className="font-medium">需要升级 Agent</div><div className="mt-1 text-default-600">远程终端要求 Agent {MIN_AGENT_VERSION} 或更高版本，当前版本为 {node.version || '未知'}。</div></div>
        </section>
      )}
      ...
      <section className="overflow-hidden rounded-lg border border-divider bg-[#111318]">
        <div className="flex h-11 items-center justify-between border-b border-white/10 px-4 text-sm text-gray-300">
          <div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${connected ? 'bg-emerald-400' : connecting ? 'bg-amber-400' : 'bg-gray-500'}`} />{sessionStatus}</div>
          <Button isIconOnly size="sm" variant="light" className="text-gray-300" aria-label="重新适配终端" onPress={() => fitAddonRef.current?.fit()}><RefreshCw size={16} /></Button>
        </div>
        <div ref={terminalContainerRef} className="h-[56vh] min-h-[420px] w-full p-3" />
      </section>

      <section className="space-y-3 border-t border-divider pt-5">
        <div className="flex items-center justify-between"><h2 className="text-base font-semibold">最近会话</h2><span className="text-xs text-default-500">仅记录会话元数据</span></div>
        {audits.length === 0 ? <div className="py-8 text-center text-sm text-default-500">暂无终端会话记录</div> : (
          <div className="overflow-x-auto rounded-lg border border-divider">
            <div className="min-w-[760px]">
              <div className="grid grid-cols-[1fr_1fr_1.2fr_1fr_1.4fr] gap-4 bg-default-100 px-4 py-3 text-xs text-default-500">
                <span>操作者</span><span>来源 IP</span><span>开始时间</span><span>状态</span><span>结束原因</span>
              </div>
              {audits.map(item => (
                <div key={item.sessionId} className="grid grid-cols-[1fr_1fr_1.2fr_1fr_1.4fr] gap-4 border-t border-divider px-4 py-3 text-sm">
                  <span>{item.username}</span><span className="font-mono text-xs">{item.sourceIp || '-'}</span>
                  <span>{new Date(item.startedAt).toLocaleString('zh-CN', { hour12: false })}</span>
                  <span>{auditStatus[item.status] || item.status}</span><span className="truncate text-default-500" title={item.closeReason}>{item.closeReason || '-'}</span>
                </div>
              ))}
            </div>
          </div>
