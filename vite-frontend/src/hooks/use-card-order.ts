...
export function useCardOrder(scope: string, itemIds: Array<string | number>) {
  ...
  const [order, setOrder] = useState<string[]>(() =>
    mergeOrder(normalizedIds, readLocalOrder(storageKey)),
  );
  const orderRef = useRef(order);
  const interactionRevisionRef = useRef(0);
  const saveQueueRef = useRef<Promise<void>>(Promise.resolve());

  const enqueueRemoteSave = useCallback(
    (nextOrder: string[]) => {
  ...
  const commitOrder = useCallback(
    (nextOrder: string[], syncRemote = true) => {
  ...
  useEffect(() => {
    ...
    void getLayoutOrder(scope)
      .then((response) => {
        if (
          !active ||
          response.code !== 0 ||
          interactionRevisionRef.current !== requestRevision
        )
          return;
        const remoteOrder = Array.isArray(response.data)
          ? normalizeIds(response.data)
          : [];
        const nextOrder = mergeOrder(
          normalizedIds,
          remoteOrder.length > 0 ? remoteOrder : localOrder,
        );
  ...
  const sortItems = useCallback(
    <T>(items: T[], getId: (item: T) => string | number): T[] => {
