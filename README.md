...
`2.24.0` 新增“家庭接入”，`2.25.0` 增加公网 IPv6 直连，`2.25.1` 在外部 IPv6 查询域名解析失败时回退到本机默认路由和网卡地址检测。现在可以按家庭网络条件选择两种接入方式：
...
3. 家庭接入端显示在线后，新建家庭代理。优先选择“IPv6 直连”，填写家庭监听端口并选择出口 VPS；没有可用 IPv6 时选择“公网中继”。
...
- IPv6 地址在页面中按 `[IPv6]:端口` 显示。面板每 5 分钟重新检测一次过期记录，每 30 秒最多处理一条，地址变化后只有通过公网可达性验证才会更新；也可以在卡片上手动重新检测。当前阶段不自动配置家庭路由器，也不为该地址创建 DNS 记录。
...
首次启用家庭接入可执行 [`migrations/20260728_home_proxy_routes.sql`](migrations/20260728_home_proxy_routes.sql)；从旧版升级 IPv6 直连可执行 [`migrations/20260729_home_proxy_ipv6_direct.sql`](migrations/20260729_home_proxy_ipv6_direct.sql)。升级只增加可空字段并将入口池字段改为可空，不删除或重写节点、隧道、转发、内网映射和现有租约。回退前应先在新版删除 IPv6 直连代理并等待卡片消失，再执行 `sudo /usr/local/sbin/flux-panel-manager rollback`；旧版会忽略新增字段，但不能识别或清理仍留在 Agent 中的直连服务。
...
`2.26.3` 修正无认证家庭代理卡片显示多余数字的问题。`2.26.4` 修正 Agent 发布标签与二进制内置版本不一致导致在线和手动升级全部失败的问题；家庭代理删除后立即从页面移除，离线接入端的残留配置继续由后台延迟清理。`2.26.5` 在 IPv6 家庭代理卡片增加“公网接入”教程，自动带出 OpenWrt 规则、当前 IPv6、监听端口和外网测试命令；同时将面板版本与 Agent 版本解耦，纯面板更新不会要求全部节点重启 Agent。
