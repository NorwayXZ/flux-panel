...
`2.24.0` 新增“家庭接入”，用于实现 `公司设备 -> 公网入口 VPS -> 家庭宽带设备 -> 指定出口 VPS -> 目标网站`。家庭宽带不需要公网 IP，也不需要在路由器开放入站端口；家庭 Agent 会主动连接面板和公网节点。

使用顺序：

1. 在“端口资源”准备两个有可用端口的端口池。第一个作为公司设备连接的公网入口，第二个属于最终访问网站的出口 VPS；两者可以来自不同服务器。
2. 进入“家庭接入”，点击“添加家庭设备”，选择 Linux、Windows 或 macOS，并在家庭设备执行页面生成的安装命令。
3. 家庭接入端显示在线后，新建家庭代理，依次选择家庭设备、公网入口端口池和家庭出口 VPS 端口池。
4. 在公司电脑的浏览器、系统代理或代理客户端中填写页面生成的 SOCKS5 地址。开启客户端认证时还需填写页面显示的用户名和密码。

- 每条家庭代理占用两个全局端口：入口池中的一个公网 SOCKS5 端口，以及出口池中的一个随机强认证网关端口。二者都进入统一端口账本，不能与转发、隧道跳点、内网映射、端口授权或其他家庭代理重复。
...
- 面板只下发配置，业务流量不经过 Spring Boot 或 MySQL，因此不会明显提高面板服务器配置要求。每条连接会经过入口 VPS、家庭设备和出口 VPS，家庭上传带宽、三段网络质量、文件描述符和出口 VPS 流量套餐都会影响实际速度与成本。

升级只新增 `home_proxy_route` 表，不改写节点、隧道、转发、内网映射或现有端口租约；手动维护数据库时可执行 [`migrations/20260728_home_proxy_routes.sql`](migrations/20260728_home_proxy_routes.sql)。回退旧版本前，应先在新版删除全部家庭代理并等待卡片消失，再执行 `sudo /usr/local/sbin/flux-panel-manager rollback`。旧版会忽略新表，但无法清理仍留在 Agent 中的家庭代理服务。
