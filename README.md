...
`2.24.0` 新增“家庭接入”，`2.25.0` 增加公网 IPv6 直连，`2.25.1` 在外部 IPv6 查询域名解析失败时回退到本机默认路由和网卡地址检测，`2.26.6` 增加公网 IPv4 直连和动态 DNS 绑定。现在可以按家庭网络条件选择三种接入方式：
...
1. 在“端口资源”准备最终出口 VPS 的端口池。只有选择公网中继时，才需要额外准备一个公网入口端口池。
...
首次启用家庭接入可执行 [`migrations/20260728_home_proxy_routes.sql`](migrations/20260728_home_proxy_routes.sql)；从旧版升级 IPv6 直连可执行 [`migrations/20260729_home_proxy_ipv6_direct.sql`](migrations/20260729_home_proxy_ipv6_direct.sql)；升级 IPv4 直连和 DDNS 绑定可执行 [`migrations/20260729_home_proxy_ipv4_ddns.sql`](migrations/20260729_home_proxy_ipv4_ddns.sql)。升级只增加可空字段并将入口池字段改为可空，不删除或重写节点、隧道、转发、内网映射和现有租约。回退前应先在新版删除 IPv4/IPv6 直连代理并等待卡片消失，再执行 `sudo /usr/local/sbin/flux-panel-manager rollback`；旧版会忽略新增字段，但不能识别或清理仍留在 Agent 中的直连服务。
