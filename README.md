...
`2.24.0` 新增“家庭接入”，`2.25.0` 增加公网 IPv6 直连，`2.26.6` 增加公网 IPv4 直连和动态 DNS，`2.27.0` 增加多级隧道出口，`2.30.0` 将单 VPS 端口池改为任意在线的“指定服务器出口”，并允许自行选择家庭到出口的 SOCKS5 或 VLESS + REALITY。现在可以按家庭网络条件选择三种接入方式：
...
3. 家庭接入端显示在线后，新建家庭代理。优先选择“IPv6 直连”；如果已经申请到真实公网 IPv4，可选择“IPv4 直连”；没有公网入站条件时选择“公网中继”。
...
首次启用家庭接入可执行 [`migrations/20260728_home_proxy_routes.sql`](migrations/20260728_home_proxy_routes.sql)；升级 Reality 指定出口可执行 [`migrations/20260729_home_proxy_reality_egress.sql`](migrations/20260729_home_proxy_reality_egress.sql)。正常容器升级会由后端自动补列。`2.30.0` 不删除或重写节点、隧道、转发、端口池及旧家庭代理；旧记录继续按标准 TCP 运行。回退前先删除在 `2.30.0` 创建的 Reality 家庭代理，确认卡片消失后执行 `sudo /usr/local/sbin/flux-panel-manager rollback`，避免旧面板无法清理 Agent 中的新 Xray 客户端运行时。
