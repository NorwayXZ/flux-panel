...
- 入口接入可按电信、联通、移动的 DNS 线路，将新连接分配到对应公网入口并自动健康回退
- 入口接入实时展示物理入口的当前连接、累计新增、流量与最近活跃时间，并记录精简的入口活动历史

### 运营商入口接入

`2.22.0` 在核心业务中新增“入口接入”。`2.22.1` 将 Cloudflare、DNSPod 和阿里云 DNS 的账号配置统一放到“DNS 与域名”，并让缺少运营商 DNS 凭据的入口接入弹窗直接打开对应添加窗口。`2.22.2` 防止浏览器密码管理器把面板登录账号误填到 DNS 密钥表单。`2.22.3` 修正阿里云运营商线路记录的最低 TTL，并在同步失败时显示阿里云返回的具体错误码。`2.22.4` 修正阿里云 DNS API 签名时间戳格式，`2.22.5` 防止已编码的阿里云 RPC 查询参数被重复编码，`2.22.6` 修正线路记录重复同步和保存时的并发冲突，`2.22.7` 正确处理阿里云对无变化更新返回的重复记录响应。`2.23.0` 增加实时入口状态与精简活动历史，`2.23.1` 将面板自身的健康探测连接从用户活动中扣除，并让共用物理入口只执行一次探测。管理员可以为一个业务域名分别选择默认、电信、联通和移动入口；DNSPod 或阿里云 DNS 根据访问者所使用的递归 DNS 线路返回相应入口地址。不同运营商用户可以同时进入不同 VPS，不需要安装客户端，也不会把所有用户反复切换到同一入口。
...
3. 在“入口接入”填写主域名和业务域名，选择默认入口，并按需要选择电信、联通、移动入口。未单独配置的运营商会使用默认记录。
...
升级只新增 `smart_entry_group`、`smart_entry_route` 和 `smart_entry_event` 三张独立表，不修改现有节点、隧道、转发和端口记录。删除入口接入策略时，面板创建的线路记录会被删除；面板接管的原有线路记录会恢复接管前的地址与 TTL。被引用的转发始终保留。手动维护数据库时可执行 [`migrations/20260728_smart_entry.sql`](migrations/20260728_smart_entry.sql)。回退旧版前建议先在新版删除入口接入策略；若直接回退，现有线路 DNS 记录会保留但不再自动健康回退。出现异常可执行 `sudo /usr/local/sbin/flux-panel-manager rollback`。
