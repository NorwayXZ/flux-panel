...
`2.22.0` 在核心业务中新增“入口接入”。`2.22.1` 将 Cloudflare、DNSPod 和阿里云 DNS 的账号配置统一放到“DNS 与域名”，并让缺少运营商 DNS 凭据的入口接入弹窗直接打开对应添加窗口。`2.22.2` 防止浏览器密码管理器把面板登录账号误填到 DNS 密钥表单。`2.22.3` 修正阿里云运营商线路记录的最低 TTL，并在同步失败时显示阿里云返回的具体错误码。`2.22.4` 修正阿里云 DNS API 签名时间戳格式，`2.22.5` 防止已编码的阿里云 RPC 查询参数被重复编码，`2.22.6` 修正线路记录重复同步和保存时的并发冲突。管理员可以为一个业务域名分别选择默认、电信、联通和移动入口；DNSPod 或阿里云 DNS 根据访问者所使用的递归 DNS 线路返回相应入口地址。不同运营商用户可以同时进入不同 VPS，不需要安装客户端，也不会把所有用户反复切换到同一入口。
