...
- 提供统一端口账本和冲突诊断，集中追踪转发入口、隧道跳点、服务发布、控制端口和用户授权
...
`2.11.0` 将租期从端口池移到每个发布服务，并增加用户端口资源授权。端口池只定义公网节点、端口范围、控制端口和释放冷却；发布服务时再选择“永久”或“定时”，因此同一端口池可以同时承载长期服务和临时服务。
...
- 普通用户只能看到管理员授予自己的端口段，发布服务时只能在该范围内自动分配或指定端口。
- 普通用户仪表板展示端口段、端口总数、已用和剩余；管理员的“服务发布”卡片标明使用账号、授权范围和具体用途。
- 永久服务不会自动到期，只有主动停止服务后端口才进入冷却并释放。定时服务到期后沿用原有离线保护与自动清理流程。
- 端口池与原有转发继续共享同一个公网端口命名空间：创建端口池时会检查已有转发，创建转发时也会排除整段端口池，避免隧道转发、直接转发和服务发布抢占同一端口。
...
`2.12.0` 将分散在转发、隧道和服务发布中的端口占用整理为管理员可查询的统一账本。“端口资源”页面可以按服务器节点、端口、记录类型和关键词筛选，并可定位一个端口被什么业务、哪个用户、哪条隧道或哪个授权范围占用。
...
- 创建转发时检查入口 TCP/UDP 端口和多级隧道每一跳的监听端口；创建端口池时检查反向控制端口；发布服务时检查最终公网端口。
- Agent `2.12.0` 使用短暂 bind 后立即关闭的方式检查操作系统真实监听，不启动常驻扫描线程，不增加持续 CPU、内存或带宽占用。
- Agent `2.11.x` 及更早版本继续使用原有数据库冲突检查，节点监控、隧道、转发和服务发布不会因面板升级而失效；升级到 `2.12.0` 后自动启用系统端口检查。
