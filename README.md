...
`2.22.0` 在核心业务中新增“入口接入”。管理员可以为一个业务域名分别选择默认、电信、联通和移动入口；DNSPod 或阿里云 DNS 根据访问者所使用的递归 DNS 线路返回相应入口地址。不同运营商用户可以同时进入不同 VPS，不需要安装客户端，也不会把所有用户反复切换到同一入口。

使用前需要满足以下条件：

1. 在“动态 DNS -> DNS 配置”保存 DNSPod 或阿里云 DNS 凭据，并确保该服务商是主域名当前实际使用的权威 DNS。Cloudflare 普通权威 DNS 不提供中国三网线路解析，因此不能用于本功能。
