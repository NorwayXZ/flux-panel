...
`2.17.0` 将“DNS 与域名”和“内网映射”连接成完整 HTTPS 链路。管理员在域名入口选择已登记的 Cloudflare Zone 后，面板会自动创建 DNS-only 的 `A` 或 `AAAA` 记录，通过 Let’s Encrypt DNS-01 申请证书，并将证书和精确域名规则下发到公网入口 Agent。内网 Web 服务只需提供普通 HTTP，不需要自己维护公网证书。
...
3. 在“内网映射 -> 域名入口”选择“面板托管 HTTPS”、对应 Zone、内网映射和公网监听端口，通常为 `443`。
