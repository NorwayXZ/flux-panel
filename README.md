...
`2.24.0` 新增“家庭接入”，`2.25.0` 增加公网 IPv6 直连，`2.25.1` 在外部 IPv6 查询域名解析失败时回退到本机默认路由和网卡地址检测，`2.26.6` 增加公网 IPv4 直连和动态 DNS 绑定，`2.27.0` 增加多级隧道出口。现在可以按家庭网络条件选择三种接入方式：

- **IPv6 直连（推荐）**：`公司设备 -> 家庭公网 IPv6 -> 家庭设备 -> 指定出口 VPS -> 目标网站`。不占用公网入口 VPS 端口，链路更短。
- **IPv4 直连**：`公司设备 -> 家庭公网 IPv4:端口 -> 家庭设备 -> 指定出口 VPS -> 目标网站`。需要真实公网 IPv4，并在家庭路由器配置端口转发。
- **公网中继（兼容）**：`公司设备 -> 公网入口 VPS -> 家庭设备 -> 指定出口 VPS -> 目标网站`。家庭网络不需要公网 IP，也不需要开放入站端口。

使用顺序：

1. 使用“单 VPS 出口”时，在“端口资源”准备最终出口 VPS 的端口池；使用“隧道出口”时直接选择现有的 2 级或更长隧道，不需要为每个路径节点额外创建端口池。只有选择公网中继时，才需要再准备一个公网入口端口池。
...
4. 在公司电脑的浏览器、系统代理或代理客户端中填写页面生成的 SOCKS5 地址。开启客户端认证时还需填写页面显示的用户名和密码。

- IPv6 直连要求公司网络能够访问 IPv6、家庭设备拥有可入站的公网 IPv6，并在家庭路由器及操作系统防火墙放行所选 TCP 端口。面板会从出口 VPS 反向验证该地址；验证失败时不会保留半成品服务或占用端口。
- 出口路径可选择“单 VPS 出口”或“隧道出口”。隧道出口的完整路径为 `公司设备 -> 家庭宽带 -> 隧道第 1 个节点 -> 中间节点 -> 最后落地节点 -> 目标网站`，最后一个节点的公网 IP 就是网站看到的出口 IP。
...
首次启用家庭接入可执行 [`migrations/20260728_home_proxy_routes.sql`](migrations/20260728_home_proxy_routes.sql)；从旧版升级 IPv6 直连可执行 [`migrations/20260729_home_proxy_ipv6_direct.sql`](migrations/20260729_home_proxy_ipv6_direct.sql)；升级 IPv4 直连和 DDNS 绑定可执行 [`migrations/20260729_home_proxy_ipv4_ddns.sql`](migrations/20260729_home_proxy_ipv4_ddns.sql)；升级多级隧道出口可执行 [`migrations/20260729_home_proxy_tunnel_egress.sql`](migrations/20260729_home_proxy_tunnel_egress.sql)。升级只增加可空字段和独立网关表，不删除或重写节点、隧道、转发、内网映射和现有租约。回退前应先在新版删除 IPv4/IPv6 直连代理及隧道出口代理并等待卡片消失，再执行 `sudo /usr/local/sbin/flux-panel-manager rollback`；旧版会忽略新增字段和表，但不能识别或清理仍留在 Agent 中的新运行配置。
